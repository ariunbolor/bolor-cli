...
# Make sure we can import from the bolor package
# regardless of whether this script is installed or run from source
# First try importing directly
try:
    from bolor.__main__ import main as bolor_main
except ImportError:
...
def main():
    ...
    # Force the command name to be 'bolor'
    if len(sys.argv) > 0:
        original_argv0 = sys.argv[0]
        sys.argv[0] = "bolor"
        
    # Call bolor main function directly
    try:
        return bolor_main()
    finally:
        # Restore original argv[0] to avoid side effects
        if len(sys.argv) > 0:
...
def main():
    ...
    print("Test with: bolor check --help")
